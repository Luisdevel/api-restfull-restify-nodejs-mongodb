...
export const authorize: (...profies: string[]) => restify.RequestHandler = (...profiles) => {
  return (req, resp, next) => {
    if (req.authenticated !== undefined && req.authenticated.hasAny(...profiles)) {
      ...
      if (req.authenticated) {
        req.log.debug('Permission danied for %s. Required profiles: %j. User profiles: %j',
          req.authenticated._id, profiles, req.authenticated.profiles);
      };

      next(new ForbiddenError('Permission danied'));
    }
  };
};
